...
abstractTable$Lastname <- abstractTable$Presenter.Name|>strsplit(" ")|>sapply(tail,1)

abstractTable <- abstractTable[order(abstractTable$Lastname),]

mdcontent <- sapply(1:nrow(abstractTable),function(i) with(abstractTable[i,],{
  paste(
    sprintf("# %s",Poster.Title),
    sprintf("\nPresenter: %s (%s)\n",Presenter.Name,Presenter.s.Institution),
    sprintf("\n%s\n",Author.list),
    sprintf("\n%s\n",Author.affiliations),
    sprintf("\n%s\n",Abstract.text),
    sprintf("\n\nKeywords: %s\n\n",Keywords)
...
con <- file("abstracts_sorted.md",open="w")
writeLines(mdcontent,con=con)
close(con)

#compile md to docx
retVal <- system("pandoc -o abstracts_sorted.docx abstracts_sorted.md")
...
con <- file("abstracts_sorted.tex",open="w")
writeLines(c(header,content,footer),con=con)
close(con)

#compile latex to pdf
retVal <- system("pdflatex -halt-on-error abstracts_sorted")
...
if (retVal == 0) {
  file.remove(c("abstracts_sorted.aux","abstracts_sorted.log","abstracts_sorted.tex"))
}
